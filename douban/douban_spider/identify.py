...
def Binarize(img, threshold):
    ...
    for x in range(w):
        for y in range(d):
            imggraydata[x, y] = 255 * (
            imgdata[x, y][0] > threshold or imgdata[x, y][1] > threshold or imgdata[x, y][2] > threshold)
    return img_G
...
def recognition(path):
    ...
    img = Binarize(img, 45)
    img = m_filter2(img)
    string_re = pytesseract.image_to_string(img)
    if string_re.find(" ") >= 0:
